...
def exibir_banner():
 ...
 An automated web penetration testing pipeline (Windows Edition).
...
def verificar_golang():
    if not shutil.which("go"):
        print("\n[!] ERRO CRITICO: O Golang nao foi detectado no sistema.")
        print("[!] Para a instalacao automatica funcionar no Windows, voce precisa ter o Go instalado.")
        print("[>] Baixe em: https://go.dev/dl/ (Instale o arquivo .msi e clique em 'Next' ate o fim).")
        print("[>] Apos instalar, feche este terminal, abra um novo e rode --setup novamente.\n")
        sys.exit(1)
...
def executar_comando(comando, arquivo_log=None, monitorar_429=False, limite_429=5, pausa_waf=120):
    ...
    processo = subprocess.Popen(
        comando, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True, bufsize=1, shell=True
    )
    contador_429 = 0
    saida_completa = []
        ...
        if monitorar_429 and psutil:
            try:
                ...
                if contador_429 >= limite_429:
                    print(f"\n[!] WAF detectado. Pausando threads por {pausa_waf}s...")
                    p = psutil.Process(processo.pid)
                    p.suspend()
                    time.sleep(pausa_waf)
                    print("[*] WAF esfriou. Retomando execução...")
                    p.resume()
                    contador_429 = 0
...
def main():
    parser = argparse.ArgumentParser(description="RECON-AI - Automação de Pentest Web (Edição para Windows)")
    parser.add_argument("--setup", action="store_true", help="Baixa e instala todas as dependências automaticamente")
    ...
    if args.target and args.ai:
        ...
        if os.path.exists(urls) and os.stat(urls).st_size > 0:
            ...
            print(f"\n[+] Pipeline finalizado. Artefatos salvos em: .\\{pasta_saida}\\")
        else:
            print("[!] Nenhuma URL viva encontrada no alvo.")
    else:
        parser.print_help()
